...
export function buildSailGeometry(shape) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const h = st.h;
    // luff(마스트) 쪽 점의 y 좌표 — foot은 boom 높이부터 시작.
    const yLuff = RIG.boomHeight + h * SAIL.luffLength;
    // 마스트 벤드: backstay tighten시 mid가 앞(+x)으로 휘는 효과. cubic 형태.
    const mastDx = shape.mastBend * (h * h * h);

    // 이 단면의 chord 방향 (xz 평면에서):
    // 메인세일은 mast에서 보트 뒤쪽(-x)으로 펼쳐진다. 기본 leech 방향: -x.
    // boomAngle > 0 일 때 leech가 leeward(-z, port=starboard tack에서 leeward)로 회전.
    // 단면 twist만큼 더 회전.
    const angle = shape.boomAngle + st.twist;
    // leech 방향 벡터 (단위) — mast(luff)에서 leech로 가는 방향
    const lx = -Math.cos(angle);              // 기본 -x (보트 뒤쪽)
    const lz = -Math.sin(angle);              // boomAngle 양수일 때 leech가 -z쪽으로

    // camber bulge 방향: chord(lx,lz)에 수직, 단면 windward의 반대(leeward)쪽으로.
    // (lx,lz) = (-cos, -sin) → 90° 시계 회전 (xz 평면에서) = (-sin, +cos) 또는 (+sin, -cos).
    // starboard tack에서 sail은 -z(port=leeward)로 부풀어야 하므로 z 성분이 음수인 방향:
    ...
    // boom rise tilt — 단면 chord 평면이 위로 기울어지는 각도.
    // foot(h=0)에서 최대(=shape.boomRise), head(h=1)에서 0 — 머리는 마스트 끝에 묶여 있음.
    ...
    for (let j = 0; j < M; j++) {
      ...
      // chord 방향의 수평 성분 거리 = t * chord * cos(tilt), 수직(y) 성분 = t * chord * sin(tilt)
      ...
      // camber bulge는 수평면 안에서 부풀어 오름 (chord가 기울어졌어도 단순화)

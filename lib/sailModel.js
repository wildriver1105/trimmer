 ...
 *     boomRise: number,           // boom이 gooseneck 기준으로 위로 들리는 각도 (rad)
...
export function buildSailShape(trim) {
  ...
  // boom rise (붐이 위로 들리는 각도):
  // - sheet=1(인)이면 sheet 자체가 boom을 당기므로 vang과 무관하게 0
  // - sheet ease + vang loose → leech 장력 무→붐 위로 들림 (최대)
  // - sheet ease + vang tight → vang이 붐을 누르므로 0에 가까움
  const boomRise = (1 - sheet) * (1 - vang) * TRIM_EFFECTS.maxBoomRise;

  // 총 twist (foot vs head AoA 차이): vang 풀수록 + sheet ease할수록 증가
  // vang=0 → 최대 twist (vangBaseTwist), vang=1 → 0
  ...
  return { boomAngle, boomRise, mastBend, stations };
}

...
export function computeForces(shape, geomData, wind) {
  ...
  // ── 세일 천(soft sail)의 동적 상태: 채워짐(fill)/펄럭임(luff)/붕괴(collapse) ──
  // 신뢰 가능한 상대풍각 AWA(뱃머리 기준) 기반의 운용범위 모델.
  //   - 상풍세일(main/jib/genoa): AWA가 너무 작으면(너무 높이 향함) 앞전이 luff. 붕괴는 없음.
  //   - 다운윈드(spinnaker/gennaker): AWA가 운용각 미만이면 맞바람 → collapse.
  ...
  if (sk === 'spinnaker' || sk === 'gennaker') {
    const lo = sk === 'spinnaker' ? 62 : 50;        // 이 AWA 미만 → 붕괴
    const fillF = smooth(lo, lo + 26, awaDeg);
    fillBase = fillF * 1.0 - (1 - fillF) * 0.7;      // 채워지면 +1, 못 차면 음수(붕괴)
    collapseBase = Math.max(0, -fillBase);
    luffBase = (1 - fillF) * smooth(lo - 22, lo, awaDeg) * 0.5;
  } else {
    const luffLo = 16, luffHi = 30;                  // AWA<16 강한 luff, >30 채워짐
    const fillF = smooth(luffLo, luffHi, awaDeg);
    fillBase = fillF;
    luffBase = 1 - fillF;
    collapseBase = 0;
    if ((sk === 'jib' || sk === 'genoa') && awaDeg > 150) {
      const bl = smooth(150, 178, awaDeg);           // 데드 다운윈드 → 메인에 가려 blanket
  ...
  for (let i = 0; i < N; i++) {
    ...
    // 세일 레벨 상태를 단면 높이로 약간 변주 (상부가 먼저/더 펄럭임 — 트위스트로 더 eased).
